...
此工作流 (Workflow) 支持用戶隨時丟入文字、網址或檔案，並將其自動化處理。若原始內容超過 3000 字，將生成 3000-5000 字的深度長文摘要，基於主題拆分成 3-5 個章節並搭配圖片生成；若少於 3000 字則直接重新改寫。最終按「周」分段 (Weekly Grouping) 且將新內容插入最上方 (Prepend) 儲存。
