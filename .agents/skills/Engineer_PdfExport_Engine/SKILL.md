---
name: Engineer_PdfExport_Engine
...
metadata:
  klio:
    requires:
      inputs: []
      skills: [Engineer_WebLayout_Builder]
    outputs: [Report_{date}.pdf, Report_{date}.html]
    pillar: tools
    downstream:
      - QA_LayoutAPI_Tester
---
...
| 方向 | Agent | 銜接方式 |
|------|-------|---------|
| ↑ 上游 | Engineering (`Engineer_WebLayout_Builder`) | 接收已渲染的分頁 DOM 進行導出 |
| ↑ 上游 | UI Designer (`UIDesigner_MuseumTheme_Builder`) | 導出時保留套用的風格 CSS |
| ← 指引 | PM Agent | 導出格式規格（尺寸、邊距、品質參數） |
| ↔ 協作 | QA (`QA_LayoutAPI_Tester`) | 驗證導出檔案的視覺完整性 |
