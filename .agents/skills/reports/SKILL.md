...
- **字體大小優化與版面清理**：
  1. **字型大小優化**：生成的 HTML 報告內文一般文字（包含段落 `<p>`、列表 `<li>`、表格格 `<td/th>`、標籤 `<label>`、按鈕 `<button>` 等）必須調整為 `14pt` 以上（可以使用 CSS 強制設定如 `.a4-page p { font-size: 14pt !important; }`），並維持易讀的行高（如 `1.6`）。
  2. **封面無意義資訊清理**：在報告的第一頁（封面頁 / HERO COVER）中，除了報告的主標題、副標題與必要導讀引言外，其他無意義的欄位與區塊，如撰寫時間、目標載體、Report by、Release Date、Target Entities 等，必須在生成時予以移除，以保持封面的乾淨與視覺美感。
