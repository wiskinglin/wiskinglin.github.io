...
| 方向 | Agent | 銜接方式 |
|------|-------|---------|
| ↑ 上游 | Data Analyst (`topic_collector`) | 讀取 `_data/topics.json` 的 pending 題目 |
| ↓ 下游 | Engineering (`gems_builder`) | 將 Markdown 轉化為 `contentEditable` 網頁元件 |
| ↓ 下游 | UI Designer (`museum_theme_builder`) | 提供內容流供博物館風格掛載 |
