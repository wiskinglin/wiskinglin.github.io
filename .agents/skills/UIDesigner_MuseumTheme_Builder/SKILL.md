---
name: UIDesigner_MuseumTheme_Builder
...
metadata:
  klio:
    requires:
      inputs: [_docs/references/WebTop50.md]
      skills: []
    outputs: [themes/_base.css, themes/{id}-{style}.css, themes/theme-switcher.js]
    pillar: themes
    downstream:
...
| ↔ 協作 | Engineering (`Engineer_WebLayout_Builder`) | 提供 Design Tokens，Builder 套用至編輯器 |
| ↔ 協作 | PMM / Writer (`Writer_DeepReport_Synthesizer`) | 接收報告內容進行風格掛載 |
| ↓ 下游 | QA (`QA_LayoutAPI_Tester`) | 提供模板供排版品質驗證 |
